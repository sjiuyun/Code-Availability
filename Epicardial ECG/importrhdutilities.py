...
def get_t_and_V(channel_name, result):
    # Find channel that corresponds to this name
    channel_found, signal_type, signal_index = find_channel_in_header(channel_name, result)
    ...
    if channel_found:
        ...
        voltage = result[signal_data_name][signal_index,:]
        return t_vector, voltage
    
    else:
        raise Exception(f"Can't get data channel {channel_name}  not found.")